...
bool Game::isValidPickupList(const string& pickupList, const int pickupFloorNum) const {
    ...
    for (int i = 0; i < pickupList.length(); i++) {
        ...
        if (temp < pickupList[i]-48) {
            temp = pickupList[i]-48;
        }
        if (direction > 0) {
            if (building.getFloorByFloorNum(pickupFloorNum).getPersonByIndex(pickupList[i]-48).getTargetFloor() - building.getFloorByFloorNum(pickupFloorNum).
                getPersonByIndex(pickupList[i]-48).getCurrentFloor() <= 0) {
                return false;
            }
        }
        if (direction < 0) {
        ...
        direction = building.getFloorByFloorNum(pickupFloorNum)
            .getPersonByIndex(pickupList[i]-48).getTargetFloor() - building.getFloorByFloorNum(pickupFloorNum).
            getPersonByIndex(pickupList[i]-48).getCurrentFloor();
    }
    if (temp >= building.getFloorByFloorNum(pickupFloorNum).getNumPeople()) {
        return false;
    }
    return true;
}
