...
private:
  ...
  private:
      Key_compare key;
  public:
      bool operator()(Pair_type p1,
                      Pair_type p2){
          return key (p1.first, p2.first);
      }
  };
...
public:
  ...
  //       using "Value_type()".
    Iterator find(const Key_type& k) const{
        Pair_type inputPair(k, Value_type());
        if (entries.end()==entries.find(inputPair)){
            return end();
        }
           return entries.find(inputPair);
    }
  ...
  // HINT: http://www.cplusplus.com/reference/map/map/operator[]/
    Value_type& operator[](const Key_type& k){
        Pair_type inputPair(k, Value_type());
        if(entries.find(inputPair)!=end()){
            return find(k)->second;
        }
        else{
            Value_type v = 0;
            Pair_type input{k, v};
            entries.insert(input);
            return find(k)->second;
        }
    }
