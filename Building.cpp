...
void Building::update(Move move){
    int pickpeople[MAX_PEOPLE_PER_FLOOR] = {};
    if (move.isPassMove() != 1) {
        if (move.isPickupMove() == 1) {
            move.copyListOfPeopleToPickup(pickpeople);
            elevators[move.getElevatorId()].serviceRequest(move.getTargetFloor());
            floors[elevators[move.getElevatorId()].getCurrentFloor()].removePeople(pickpeople, move.getNumPeopleToPickup());
        }
        else if (move.isPickupMove() != 1) {
            elevators[move.getElevatorId()].serviceRequest(move.getTargetFloor());
        }
    }
}
...
int Building::tick(Move move){
    ...
    for (int j = 0; j < NUM_FLOORS; j++) {
        numofexplode+=floors[j].tick(time);
    }
    return numofexplode;
}
